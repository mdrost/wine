...
static LPWSTR ansi_to_unicode_list(LPCSTR ansi_list)
{
    ...
    list = heap_alloc(wlen * sizeof(WCHAR));
                               ...
                               DWORD dwFlags, DWORD dwReserved)
{
    ...
    if (lpszDestFile)
    {
        ...
        szDestFilename = heap_alloc((dwLen+1) * sizeof(WCHAR));
...
static LPSTR convert_file_list(LPCSTR FileList, DWORD *dwNumFiles)
{
    ...
    szConvertedList = heap_alloc(dwLen);
                                     ...
                                     LPDWORD pdwLSVer, BOOL bVersion )
{
    ...
    *pdwMSVer = 0;
    ...
    pVersionInfo = heap_alloc(dwInfoSize);
    if (!pVersionInfo)
        goto done;
