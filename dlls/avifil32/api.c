			    ...
			    LPCLSID lpHandler)
{
  ...
  wszFile = heap_alloc(len * sizeof(WCHAR));
  if (wszFile == NULL)
...
HRESULT WINAPI AVIBuildFilterA(LPSTR szFilter, LONG cbFilter, BOOL fSaving)
{
  ...
  wszFilter = heap_alloc(cbFilter * sizeof(WCHAR));
  if (wszFilter == NULL)
    return AVIERR_MEMORY;
...
HRESULT WINAPI AVIBuildFilterW(LPWSTR szFilter, LONG cbFilter, BOOL fSaving)
{
  ...
  lp = heap_alloc_zero(MAX_FILTERS * sizeof(AVIFilter));
  if (lp == NULL)
    return AVIERR_MEMORY;
...
static BOOL AVISaveOptionsFmtChoose(HWND hWnd)
{
  ...
  if (sInfo.fccType == streamtypeVIDEO) {
    ...
    afmtc.pwfxEnum = heap_alloc(size);
			   ...
			   PAVISTREAM *ppavi, LPAVICOMPRESSOPTIONS *ppOptions)
{
  ...
  if (nStreams > 1) {
    pSavedOptions = heap_alloc(nStreams * sizeof(AVICOMPRESSOPTIONS));
	...
	while (lFirstVideo > lStart[curStream]) {
	  ...
	  do {
	    hres = AVIStreamRead(pInStreams[curStream], lStart[curStream],
		   ...
		   (lpBuffer = heap_realloc(lpBuffer, cbBuffer *= 2)) != NULL);
	  if (lpBuffer == NULL)
	    hres = AVIERR_MEMORY;
	  if (FAILED(hres))
	    goto error;
                        ...
                        int nStreams, PAVISTREAM pavi, LPAVICOMPRESSOPTIONS lpOptions, ...)
{
    ...
    streams = heap_alloc(nStreams * sizeof(*streams));
    options = heap_alloc(nStreams * sizeof(*options));
    if (!streams || !options)
    {
        ret = AVIERR_MEMORY;
        goto error;
