...
static HRESULT WINAPI IAVIFile_fnDeleteStream(IAVIFile *iface, DWORD fccType, LONG lParam)
{
  ...
  if (nStream < This->fInfo.dwStreams &&
      This->ppStreams[nStream] != NULL) {
    /* ... so delete it now */
    heap_free(This->ppStreams[nStream]);
    This->fInfo.dwStreams--;
    if (nStream < This->fInfo.dwStreams)
...
static HRESULT WINAPI IPersistFile_fnLoad(IPersistFile *iface, LPCOLESTR pszFileName, DWORD dwMode)
{
  ...
  This->szFileName = heap_alloc(len * sizeof(WCHAR));
  ...
  if (This->hmmio == NULL) {
    ...
    szFileName = heap_alloc(len * sizeof(CHAR));
    if (szFileName == NULL)
      return AVIERR_MEMORY;
...
HRESULT AVIFILE_CreateAVIFile(IUnknown *pUnkOuter, REFIID riid, void **ppv)
{
  ...
  *ppv = NULL;
  obj = heap_alloc_zero(sizeof(IAVIFileImpl));
  if (!obj)
    return AVIERR_MEMORY;
    ...
    LONG formatsize)
{
  ...
  if (This->lpFormat == NULL) {
    ...
    This->lpFormat = heap_alloc(formatsize);
    if (This->lpFormat == NULL)
      return AVIERR_MEMORY;
...
static HRESULT WINAPI IAVIStream_fnWriteData(IAVIStream *iface, DWORD fcc, void *lp, LONG size)
{
  ...
  if (fcc == ckidSTREAMHANDLERDATA) {
    ...
    This->lpHandlerData = heap_alloc(size);
    if (This->lpHandlerData == NULL)
      return AVIERR_MEMORY;
...
static HRESULT AVIFILE_AddFrame(IAVIStreamImpl *This, DWORD ckid, DWORD size, DWORD offset, DWORD flags)
{
  ...
  case cktypePALchange:
	...
	This->idxFmtChanges =
          heap_alloc_zero(new_count * sizeof(AVIINDEXENTRY));
  ...
  if (This->idxFrames == NULL || This->lLastFrame + 1 >= This->nIdxFrames) {
    ...
    if (This->idxFrames == NULL)
      This->idxFrames = heap_alloc_zero(This->nIdxFrames * sizeof(AVIINDEXENTRY));
      else
...
static void AVIFILE_ConstructAVIStream(IAVIFileImpl *paf, DWORD nr, const AVISTREAMINFOW *asi)
{
  ...
  if (asi != NULL) {
    ...
    if (asi->dwLength > 0) {
      ...
      pstream->idxFrames =
	heap_alloc_zero(asi->dwLength * sizeof(AVIINDEXENTRY));
	...
	heap_alloc_zero(asi->dwFormatChangeCount * sizeof(AVIINDEXENTRY));
...
static void    AVIFILE_DestructAVIStream(IAVIStreamImpl *This)
{
  ...
  This->paf = NULL;
  if (This->idxFrames != NULL) {
    heap_free(This->idxFrames);
    This->idxFrames  = NULL;
    This->nIdxFrames = 0;
  }
  heap_free(This->idxFmtChanges);
  This->idxFmtChanges = NULL;
  if (This->lpBuffer != NULL) {
    heap_free(This->lpBuffer);
    This->lpBuffer = NULL;
    This->cbBuffer = 0;
  }
  if (This->lpHandlerData != NULL) {
    heap_free(This->lpHandlerData);
    This->lpHandlerData = NULL;
    This->cbHandlerData = 0;
  }
  if (This->extra.lp != NULL) {
    heap_free(This->extra.lp);
    This->extra.lp = NULL;
    This->extra.cb = 0;
  }
  if (This->lpFormat != NULL) {
    heap_free(This->lpFormat);
    This->lpFormat = NULL;
    This->cbFormat = 0;
  }
}
...
static HRESULT AVIFILE_LoadFile(IAVIFileImpl *This)
{
  ...
  for (nStream = 0; nStream < This->fInfo.dwStreams; nStream++) {
	...
	heap_alloc_zero(sizeof(IAVIStreamImpl));
      if (pStream == NULL)
	return AVIERR_MEMORY;
	...
	case ckidSTREAMHANDLERDATA:
	  if (pStream->lpHandlerData != NULL)
	    return AVIERR_BADFORMAT;
	  pStream->lpHandlerData = heap_alloc(ck.cksize);
	  if (pStream->lpHandlerData == NULL)
	    return AVIERR_MEMORY;
	...
	case ckidSTREAMFORMAT:
	  ...
	  pStream->lpFormat = heap_alloc(ck.cksize);
	  if (pStream->lpFormat == NULL)
	    return AVIERR_MEMORY;
	...
	case ckidSTREAMNAME:
	  { /* streamname will be saved as ASCII string */
	    LPSTR str = heap_alloc(ck.cksize);
	    if (str == NULL)
	      return AVIERR_MEMORY;
...
static HRESULT AVIFILE_LoadIndex(const IAVIFileImpl *This, DWORD size, DWORD offset)
{
  ...
  lp = heap_alloc(IDX_PER_BLOCK * sizeof(AVIINDEXENTRY));
  if (lp == NULL)
    return AVIERR_MEMORY;
  ...
  for (n = 0; n < This->fInfo.dwStreams; n++) {
    ...
    pStream->idxFrames =
      heap_alloc_zero(pStream->nIdxFrames * sizeof(AVIINDEXENTRY));
				 ...
				 LPVOID buffer, DWORD size)
{
  ...
  if (buffer == NULL) {
    ...
    if (This->lpBuffer == NULL || This->cbBuffer < size) {
      ...
      if (This->lpBuffer == NULL) {
	This->lpBuffer = heap_alloc(maxSize);
        if (!This->lpBuffer) return AVIERR_MEMORY;
      } else {
        void *new_buffer = heap_realloc(This->lpBuffer, maxSize);
        if (!new_buffer) return AVIERR_MEMORY;
