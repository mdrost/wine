 ...
 LPCSTR sigOID, const BYTE *sig, DWORD sigLen)
{
    ...
    if (cert)
    {
        ...
        pubKeyInfo = heap_alloc(pubKeySize);
        if (pubKeyInfo)
        {
...
static void testCreateSelfSignCert(void)
{
    ...
    if (ret)
    {
        ...
        if (context)
        {
            ...
            if (size)
            {
                PCRYPT_KEY_PROV_INFO pInfo = heap_alloc(size);
...
static void testAcquireCertPrivateKey(void)
{
    ...
    if (ret)
    {
        ...
        if (ret)
        {
            LPBYTE buf = heap_alloc(size), encodedKey;
    ...
    if (ret)
    {
        PCERT_PUBLIC_KEY_INFO info = heap_alloc(size);
