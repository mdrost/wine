        ...
        if (WTHelperGetKnownUsages(1, &usages))
        {
            ...
            if ((disabledUsagesStr = get_cert_mgr_usages()))
            {
                ...
                if (disabledUsages)
                {
                    ...
                    heap_free(disabledUsages);
                }
                heap_free(disabledUsagesStr);
            }
...
static void show_store_certs(HWND hwnd, HCERTSTORE store)
{
    ...
    if (advanced)
    {
        heap_free(advanced->rgpszUsageIdentifier);
        heap_free(advanced);
    }
 ...
 LPARAM lp)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        data = heap_alloc(sizeof(struct CertMgrData));
        if (!data)
            return 0;
...
HCERTSTORE WINAPI CryptUIDlgSelectStoreA(PCRYPTUI_SELECTSTORE_INFO_A info)
{
    ...
    if (info->pszTitle)
    {
        ...
        infoW.pwszTitle = heap_alloc(len * sizeof(WCHAR));
 ...
 BOOL *anyUsageAdded)
{
    ...
    if (CertGetEnhancedKeyUsage(cert, 0, NULL, &size))
    {
        ...
        PCERT_ENHKEY_USAGE usage = heap_alloc(size);
...
static WCHAR *get_cps_str_from_qualifier(const CRYPT_OBJID_BLOB *qualifier)
{
     ...
     &qualifierValue, &size))
    {
        ...
        qualifierStr = heap_alloc(size * sizeof(WCHAR));
        if (qualifierStr)
         ...
         CRYPT_DECODE_ALLOC_FLAG, NULL, &policies, &size))
        {
            ...
            if (cps || userNotice)
            {
                struct IssuerStatement *issuerStatement =
                 heap_alloc(sizeof(struct IssuerStatement));
 ...
 PROPSHEETPAGEW *page)
{
    ...
    case PSPCB_RELEASE:
        ...
        if (issuerStatement)
        {
            heap_free(issuerStatement->cps);
            heap_free(issuerStatement->userNotice);
            heap_free(issuerStatement);
        }
        break;
    }
    return 1;
...
static WCHAR *field_format_version(PCCERT_CONTEXT cert)
{
    ...
    WCHAR *buf = heap_alloc(12 * sizeof(WCHAR));
...
{
    WCHAR *buf = heap_alloc((cb * 3 + 1) * sizeof(WCHAR));
...
{
    if (data->cFields)
        data->fields = heap_realloc(data->fields,
 ...
 PCERT_EXTENSION ext)
{
    ...
    else
    {
        ...
        LPWSTR oidW = heap_alloc((len + 1) * sizeof(WCHAR));
...
static void add_purpose(HWND hwnd, LPCSTR oid)
{
    ...
    if (info)
    {
        char *oidCopy = heap_alloc(strlen(oid) + 1);
...
static void apply_general_changes(HWND hwnd)
{
    ...
    else if (IsDlgButtonChecked(hwnd, IDC_ENABLE_SELECTED_PURPOSES))
    {
        ...
        CertSetEnhancedKeyUsage(data->cert, &usage);
        heap_free(usage.rgpszUsageIdentifier);
    }
...
static void free_detail_fields(struct detail_data *data)
{
    ...
    for (i = 0; i < data->cFields; i++)
        heap_free(data->fields[i].detailed_value);
    heap_free(data->fields);
    data->fields = NULL;
    data->cFields = 0;
}
 ...
 CRYPT_PROVIDER_CERT *provCert, BOOL *pfPropertiesChanged)
{
    ...
    if (!(pCertViewInfo->dwFlags & CRYPTUI_HIDE_HIERARCHYPAGE))
        nPages++;
    pages = heap_alloc(nPages * sizeof(PROPSHEETPAGEW));
    if (pages)
    {
        PROPSHEETHEADERW hdr;
...
static WCHAR *make_import_file_filter(DWORD dwFlags)
{
    ...
    filter = heap_alloc(totalLen * sizeof(WCHAR));
    if (filter)
    {
        LPWSTR ptr;
 ...
 LPARAM lp)
{
    ...
    case WM_COMMAND:
        ...
        case IDC_IMPORT_BROWSE_FILE:
        {
            ...
            heap_free((LPWSTR)ofn.lpstrFilter);
            break;
        }
        }
        break;
 ...
 LPWSTR fileName)
{
    ...
    if (appendExtension)
    {
        fileName = heap_realloc(fileName,
 ...
 PCRYPT_KEY_PROV_INFO keyProvInfo, BOOL deleteKeys)
{
    ...
    if (store)
    {
        ...
        if (ret)
        {
            ...
            if (ret)
            {
                pfxBlob.pbData = heap_alloc(pfxBlob.cbData);
                if (pfxBlob.pbData)
                {
                    ret = PFXExportCertStore(store, &pfxBlob,
                     pContextInfo->pwszPassword, exportFlags);
