...
static void free_signature(struct d3dcompiler_shader_signature *sig)
{
    ...
    heap_free(sig->elements);
    heap_free(sig->string_data);
}
...
static void reflection_cleanup(struct d3dcompiler_shader_reflection *ref)
{
    ...
    heap_free(ref->constant_buffers);
    heap_free(ref->bound_resources);
    heap_free(ref->resource_string);
    heap_free(ref->creator);
}
...
static HRESULT d3dcompiler_parse_type(struct d3dcompiler_shader_reflection_type *type, const char *data, DWORD offset)
{
    ...
    if (desc->Members)
    {
        ...
        members = heap_alloc_zero(sizeof(*members) * desc->Members);
        if (!members)
        {
...
static struct d3dcompiler_shader_reflection_type *get_reflection_type(struct d3dcompiler_shader_reflection *reflection, const char *data, DWORD offset)
{
    ...
    type = heap_alloc_zero(sizeof(*type));
    if (!type)
        return NULL;
        ...
        const char *data, DWORD data_size, const char *ptr)
{
    ...
    variables = heap_alloc_zero(cb->variable_count * sizeof(*variables));
    if (!variables)
    {
...
static HRESULT d3dcompiler_parse_rdef(struct d3dcompiler_shader_reflection *r, const char *data, DWORD data_size)
{
    ...
    if (r->bound_resource_count)
    {
        ...
        string_data = heap_alloc(string_data_size);
        if (!string_data)
        {
        ...
        bound_resources = heap_alloc(r->bound_resource_count * sizeof(*bound_resources));
        if (!bound_resources)
        {
    ...
    {
        constant_buffers = heap_alloc_zero(r->constant_buffer_count * sizeof(*constant_buffers));
        if (!constant_buffers)
        {
...
err_out:
    ...
    heap_free(constant_buffers);
    heap_free(bound_resources);
    heap_free(string_data);
    heap_free(creator);
        ...
        const void *data, SIZE_T data_size)
{
    ...
    for (i = 0; i < src_dxbc.count; ++i)
    {
        ...
        switch (section->tag)
        {
            ...
            case TAG_ISGN:
                reflection->isgn = heap_alloc_zero(sizeof(*reflection->isgn));
                if (!reflection->isgn)
                {
            ...
            case TAG_OSG5:
            case TAG_OSGN:
                reflection->osgn = heap_alloc_zero(sizeof(*reflection->osgn));
                if (!reflection->osgn)
                {
            ...
            case TAG_PCSG:
                reflection->pcsg = heap_alloc_zero(sizeof(*reflection->pcsg));
                if (!reflection->pcsg)
                {
