...
{
    if (!modes)
        modes = heap_alloc((modes_size = 2) * sizeof(DDSURFACEDESC));
    if (modes_cnt == modes_size)
        modes = heap_realloc(modes, (modes_size *= 2) * sizeof(DDSURFACEDESC));
    assert(modes);
...
static void testddraw7(void)
{
    ...
    if (hr==DD_OK)
    {
         ...
         pdddi2 = heap_alloc( dddi2Bytes + 2*sizeof(DWORD) );
