...
static BOOL DP_CreateDirectPlay2( LPVOID lpDP )
{
  ...
  This->dp2 = heap_alloc_zero( sizeof( *(This->dp2) ) );
  if ( This->dp2 == NULL )
  {
    return FALSE;
  }
  ...
  This->dp2->spData.lpCB = heap_alloc_zero(
  ...
  This->dp2->dplspData.lpCB = heap_alloc_zero(
...
static BOOL DP_DestroyDirectPlay2( LPVOID lpDP )
{
  ...
  heap_free( This->dp2->dplspData.lpCB);
  heap_free( This->dp2->lpSessionDesc );
        ...
        void **lplpReply, DWORD *lpdwMsgSize )
{
  ...
  switch( wCommandId )
  {
    ...
    case DPMSGCMD_REQUESTNEWPLAYERID:
    {
      ...
      *lplpReply = heap_alloc_zero( *lpdwMsgSize );
        ...
        DPID player )
{
    ...
    newplist = heap_alloc_zero( sizeof( *newplist ) );
    if ( !newplist )
        ...
        DWORD dwFlags, DPID idParent, BOOL bAnsi )
{
  ...
  lpGData = heap_alloc_zero( sizeof( *lpGData ) );
        ...
        DPNAME *lpGroupName, void *lpData, DWORD dwDataSize, DWORD dwFlags, BOOL bAnsi )
{
  ...
  else
  {
    ...
    lpGroupList lpGroup = heap_alloc_zero( sizeof( *lpGroup ) );
                 ...
                 LPVOID lpData, DWORD dwDataSize )
{
  ...
  else
  {
    ...
    {
      heap_free( lpGData->lpRemoteData );
  ...
  if( lpData != NULL )
  {
    ...
    {
      lpGData->lpRemoteData = heap_alloc( dwDataSize );
        ...
        DWORD dwFlags, HANDLE hEvent, BOOL bAnsi )
{
  ...
  lpPData = heap_alloc_zero( sizeof( *lpPData ) );
  if( lpPData == NULL )
  {
    return NULL;
  }
...
static BOOL DP_CopyDPNAMEStruct( LPDPNAME lpDst, const DPNAME *lpSrc, BOOL bAnsi )
{
  ...
  heap_free( lpDst->u1.lpszShortNameA );
  heap_free( lpDst->u2.lpszLongNameA );
                  ...
                  LPVOID lpData, DWORD dwDataSize )
{
  ...
  else
  {
    ...
    {
      heap_free( lpPData->lpRemoteData );
  ...
  if( lpData != NULL )
  {
    ...
    {
      lpPData->lpRemoteData = heap_alloc( dwDataSize );
        ...
        BOOL bAnsi )
{
  ...
  lpPList = heap_alloc_zero( sizeof( *lpPList ) );
  if( !lpPData || !lpPList )
  {
    heap_free( lpPData );
    heap_free( lpPList );
        ...
        DWORD dwFlags, BOOL bInitial, BOOL bAnsi  )
{
  ...
  lpTempSessDesc = heap_alloc_zero( dwRequiredSize );
        ...
        DPID group )
{
    ...
    /* Create a player list (ie "shortcut" ) */
    glist = heap_alloc_zero( sizeof( *glist ) );
    if ( !glist )
        return DPERR_CANTADDPLAYER;
        ...
        BOOL bAnsi )
{
  ...
  lpGList = heap_alloc_zero( sizeof( *lpGList ) );
  if( lpGList == NULL )
  {
    FIXME( "Memory leak\n" );
    return DPERR_CANTADDPLAYER; /* yes player not group */
        ...
        DWORD dwFlags )
{
  ...
  if( dwFlags & DPCONNECTION_DIRECTPLAYLOBBY )
  {
         ...
         ++dwIndex, sizeOfSubKeyName=51 )
    {
      ...
      /* Now allocate the buffer */
      lpAddressBuffer = heap_alloc_zero( dwAddressBufferSize );
    ...
    *lpbIsDpSp = (i == 0);
         ...
         ++dwIndex, sizeOfSubKeyName=51 )
    {
      ...
      if( i == 0 ) /* DP SP */
      {
        ...
        lpSpData->lpszName = heap_alloc( len*sizeof(WCHAR) );
        ...
        DWORD dwPriority, DWORD dwTimeout, void *lpContext, DWORD *lpdwMsgID )
{
  ...
  lpMElem = heap_alloc_zero( sizeof( *lpMElem ) );
  lpMElem->msg = heap_alloc_zero( dwDataSize );
                                     ...
                                     LPVOID lpContext)
{
    ...
    {
	heap_free(guid_cache);
	guid_cache = heap_alloc(sizeof(GUID) * dwIndex);
	if (!guid_cache)
	{
	...
	if (lpEnumCallbackA)
	{
	    ...
	    {
		heap_free(descriptionA);
		max_sizeOfDescriptionA = sizeOfDescription;
	    }
	    descriptionA = heap_alloc(sizeOfDescription);
	...
	else
	{
	    ...
	    {
		heap_free(descriptionW);
		max_sizeOfDescriptionW = sizeOfDescription;
	    }
	    descriptionW = heap_alloc(sizeOfDescription);
