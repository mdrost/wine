...
static void testGetIpAddrTable(void)
{
  if (pGetIpAddrTable) {
    ...
    if (apiReturn == ERROR_INSUFFICIENT_BUFFER) {
      PMIB_IPADDRTABLE buf = heap_alloc(dwSize);
...
static void testGetIfTable(void)
{
  if (pGetIfTable) {
    ...
    ok(apiReturn == ERROR_INSUFFICIENT_BUFFER,
     ...
     apiReturn);
    if (apiReturn == ERROR_INSUFFICIENT_BUFFER) {
      PMIB_IFTABLE buf = heap_alloc(dwSize);
...
static void testGetIpForwardTable(void)
{
  if (pGetIpForwardTable) {
    ...
    ok(apiReturn == ERROR_INSUFFICIENT_BUFFER,
     ...
     apiReturn);
    if (apiReturn == ERROR_INSUFFICIENT_BUFFER) {
      PMIB_IPFORWARDTABLE buf = heap_alloc(dwSize);
...
static void testGetIpNetTable(void)
{
  if (pGetIpNetTable) {
    ...
    if (apiReturn == ERROR_NO_DATA)
      ...
      PMIB_IPNETTABLE buf = heap_alloc(dwSize);
...
static void testGetTcpTable(void)
{
  if (pGetTcpTable) {
    ...
    ok(apiReturn == ERROR_INSUFFICIENT_BUFFER ||
     ...
     apiReturn);
    if (apiReturn == ERROR_INSUFFICIENT_BUFFER) {
      PMIB_TCPTABLE buf = heap_alloc(dwSize);
...
static void testGetUdpTable(void)
{
  if (pGetUdpTable) {
    ...
    ok(apiReturn == ERROR_INSUFFICIENT_BUFFER,
     ...
     apiReturn);
    if (apiReturn == ERROR_INSUFFICIENT_BUFFER) {
      PMIB_UDPTABLE buf = heap_alloc(dwSize);
...
static void testGetInterfaceInfo(void)
{
  if (pGetInterfaceInfo) {
    ...
    ok(apiReturn == ERROR_INSUFFICIENT_BUFFER,
     ...
     apiReturn);
    if (apiReturn == ERROR_INSUFFICIENT_BUFFER) {
      PIP_INTERFACE_INFO buf = heap_alloc(len);
...
static void testGetAdaptersInfo(void)
{
  if (pGetAdaptersInfo) {
    ...
    if (apiReturn == ERROR_NO_DATA)
      ...
      PIP_ADAPTER_INFO ptr, buf = heap_alloc(len);
...
static void testGetNetworkParams(void)
{
  if (pGetNetworkParams) {
    ...
    if (apiReturn == ERROR_BUFFER_OVERFLOW) {
      PFIXED_INFO buf = heap_alloc(len);
