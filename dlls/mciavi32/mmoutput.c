...
static BOOL MCIAVI_GetInfoAudio(WINE_MCIAVI* wma, const MMCKINFO* mmckList, MMCKINFO *mmckStream)
{
    ...
    wma->lpWaveFormat = heap_alloc(mmckInfo.cksize);
    if (!wma->lpWaveFormat) {
...
static BOOL MCIAVI_GetInfoVideo(WINE_MCIAVI* wma, const MMCKINFO* mmckList, MMCKINFO* mmckStream)
{
    ...
    wma->inbih = heap_alloc(mmckInfo.cksize);
    if (!wma->inbih) {
				...
				struct AviListBuild* alb)
{
	...
	if (wma->lpWaveFormat) {
	    if (alb->numAudioBlocks >= alb->numAudioAllocated) {
                ...
                if (!wma->lpAudioIndex)
                    newindex = heap_alloc(newsize * sizeof(struct MMIOPos));
                else
                    newindex = heap_realloc(wma->lpAudioIndex, newsize * sizeof(struct MMIOPos));
...
BOOL MCIAVI_GetInfo(WINE_MCIAVI* wma)
{
    ...
    wma->indata = heap_alloc(wma->ash_video.dwSuggestedBufferSize);
    if (!wma->indata) {
...
BOOL    MCIAVI_OpenVideo(WINE_MCIAVI* wma)
{
    ...
    wma->outdata = heap_alloc(wma->outbih->biSizeImage);
    if (!wma->outdata) {
