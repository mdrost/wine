        ...
        TfEditCookie ec, ULONG ulCount, const TF_SELECTION *pSelection)
{
    ...
    acp = heap_alloc(sizeof(TS_SELECTION_ACP) * ulCount);
    if (!acp)
        ...
        DWORD dwLockFlags)
{
    ...
    cookie = heap_alloc(sizeof(EditCookie));
    if (!cookie)
        return E_OUTOFMEMORY;

    sinkcookie = heap_alloc(sizeof(EditCookie));
    if (!sinkcookie)
    {
        heap_free(cookie);
        return E_OUTOFMEMORY;
...
HRESULT Context_Constructor(TfClientId tidOwner, IUnknown *punk, ITfDocumentMgr *mgr, ITfContext **ppOut, TfEditCookie *pecTextStore)
{
    ...
    This = heap_alloc_zero(sizeof(Context));
    if (This == NULL)
        return E_OUTOFMEMORY;
