...
static HRESULT WINAPI ThreadMgr_CreateDocumentMgr(ITfThreadMgrEx *iface, ITfDocumentMgr **ppdim)
{
    ...
    TRACE("(%p)\n",iface);
    mgrentry = heap_alloc(sizeof(DocumentMgrEntry));
    if (mgrentry == NULL)
...
ITfDocumentMgr *pdimNew, ITfDocumentMgr **ppdimPrev)
{
    ...
    *ppdimPrev = NULL;
    ...
    wnd = heap_alloc(sizeof(AssociatedWindow));
    wnd->hwnd = hwnd;
    wnd->docmgr = pdimNew;
        ...
        const WCHAR *pchDesc, ULONG cchDesc)
{
    ...
    newkey = heap_alloc(sizeof(PreservedKey));
    if (!newkey)
        return E_OUTOFMEMORY;
    ...
    if (cchDesc)
    {
        newkey->description = heap_alloc(cchDesc * sizeof(WCHAR));
...
HRESULT ThreadMgr_Constructor(IUnknown *pUnkOuter, IUnknown **ppOut)
{
    ...
    This = heap_alloc_zero(sizeof(ThreadMgr));
    if (This == NULL)
        return E_OUTOFMEMORY;
...
static HRESULT EnumTfDocumentMgr_Constructor(struct list* head, IEnumTfDocumentMgrs **ppOut)
{
    ...
    This = heap_alloc_zero(sizeof(EnumTfDocumentMgr));
    if (This == NULL)
        return E_OUTOFMEMORY;
