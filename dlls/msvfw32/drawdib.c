...
HDRAWDIB VFWAPI DrawDibOpen(void) 
{
    ...
    whdd = heap_alloc_zero(sizeof(WINE_HDD));
    TRACE("=> %p\n", whdd);
...
BOOL VFWAPI DrawDibEnd(HDRAWDIB hdd) 
{
    ...
    heap_free(whdd->lpbi);
    whdd->lpbi = NULL;
    heap_free(whdd->lpbiOut);
    whdd->lpbiOut = NULL;

    whdd->begun = FALSE;

    /*if (whdd->lpvbits)
      heap_free(whdd->lpvbuf);*/
                         ...
                         UINT     wFlags) 
{
    ...
    else 
    {
        ...
        else
        {
            ...
            whdd->lpbiOut = heap_alloc(dwSize);
    ...
    if (ret) 
    {
        /*whdd->lpvbuf = heap_alloc(whdd->lpbiOut->biSizeImage);*/
    ...
    if (ret) 
    {
        ...
        whdd->dxDst = dxDst;
        whdd->dyDst = dyDst;
        whdd->lpbi = heap_alloc(lpbi->biSize);
