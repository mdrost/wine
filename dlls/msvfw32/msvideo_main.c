...
BOOL VFWAPI ICInstall(DWORD fccType, DWORD fccHandler, LPARAM lParam, LPSTR szDesc, UINT wFlags) 
{
    ...
    /* Register the driver */
    driver = heap_alloc_zero(sizeof(reg_driver));
    if (!driver) goto oom;
	...
	driver->proc = NULL;
        ...
        driver->name = heap_alloc(len * sizeof(WCHAR));
...
HIC VFWAPI ICOpen(DWORD fccType, DWORD fccHandler, UINT wMode) 
{
    ...
    whic = heap_alloc(sizeof(WINE_HIC));
    if (!whic)
    {
...
static BOOL enum_compressors(HWND list, COMPVARS *pcv, BOOL enum_all)
{
    ...
    while (ICInfo(pcv->fccType, id, &icinfo))
    {
        ...
        if (hic)
        {
            ...
            ic = heap_alloc(sizeof(struct codec_info));
            ic->icinfo = icinfo;
            ic->hic = hic;
	...
	LPVOID lpBits, LPBITMAPINFO lpbiOut)
{
	...
	else
	{
		...
		if ( cbHdr < sizeof(BITMAPINFOHEADER) )
			goto err;
		pHdr = heap_alloc_zero(cbHdr+sizeof(RGBQUAD)*256);
		if ( pHdr == NULL )
			goto err;
...
BOOL VFWAPI ICSeqCompressFrameStart(PCOMPVARS pc, LPBITMAPINFO lpbiIn)
{
    ...
    DWORD ret;
    ICCOMPRESS* icComp;
    pc->lpbiIn = heap_alloc(sizeof(BITMAPINFO));
    if (!pc->lpbiIn)
        return FALSE;

    *pc->lpbiIn = *lpbiIn;

    pc->lpState = heap_alloc(sizeof(ICCOMPRESS)
    ...
    if (!pc->lpbiOut)
    {
        ...
        pc->lpbiOut = heap_alloc_zero(size);
        if (!pc->lpbiOut)
            goto error;
    ...
    pc->lpBitsOut = heap_alloc(pc->lpbiOut->bmiHeader.biSizeImage);
    if (!pc->lpBitsOut)
        goto error;

    /* Buffer for previous compressed frame data */
    pc->lpBitsPrev = heap_alloc(pc->lpbiOut->bmiHeader.biSizeImage);
    if (!pc->lpBitsPrev)
        goto error;
