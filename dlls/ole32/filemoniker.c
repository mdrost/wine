...
FileMonikerImpl_Load(IMoniker* iface, IStream* pStm)
{
    ...
    if (!dwbuffer) /* No W-string */
    {
        ...
        filePathW=heap_alloc((len+1)*sizeof(WCHAR));
        if (!filePathW)
        {
            res = E_OUTOFMEMORY;
            goto fail;
    ...
    *ppmkComposite=0;
    ...
    if(mkSys==MKSYS_FILEMONIKER){
        ...
        if ((lastIdx1==-1 && lastIdx2>-1)||(lastIdx1==1 && lstrcmpW(strDec1[0],twoPoint)==0))
            ...
            newStr=heap_alloc(sizeof(WCHAR)*(lstrlenW(str1)+lstrlenW(str2)+1));
...
FileMonikerImpl_RelativePathTo(IMoniker* iface,IMoniker* pmOther, IMoniker** ppmkRelPath)
{
    ...
    /* by "..\\" in the begin */
    relPath=heap_alloc(sizeof(WCHAR)*(1+lstrlenW(str1)+lstrlenW(str2)));
...
static HRESULT FileMonikerImpl_Construct(FileMonikerImpl* This, LPCOLESTR lpszPathName)
{
    ...
    This->filePathName=heap_alloc(sizeof(WCHAR)*(sizeStr+1));
    ...
    if (nb > 0 ){
        ...
        This->filePathName=heap_realloc(This->filePathName,(sizeStr+1)*sizeof(WCHAR));
...
HRESULT WINAPI CreateFileMoniker(LPCOLESTR lpszPathName, IMoniker **ppmk)
{
    ...
    *ppmk=NULL;
    ...
    newFileMoniker = heap_alloc(sizeof(FileMonikerImpl));
                                          ...
                                          LPDWORD pchEaten, IMoniker **ppmk)
{
         ...
         end = memrpbrkW(szDisplayName, end - szDisplayName, wszSeparators))
    {
        ...
        file_display_name = heap_alloc((len + 1) * sizeof(WCHAR));
        ...
        full_path_name = heap_alloc(full_path_name_len * sizeof(WCHAR));
        if (!full_path_name)
        {
            heap_free(file_display_name);
            IMoniker_Release(file_moniker);
