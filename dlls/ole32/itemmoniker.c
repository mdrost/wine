...
static HRESULT WINAPI ItemMonikerImpl_Load(IMoniker* iface,IStream* pStm)
{
    ...
    if (!(itemNameA=heap_alloc(nameLength)))
        return E_OUTOFMEMORY;
    ...
    This->itemName=heap_realloc(This->itemName,lenW*sizeof(WCHAR));
    if (!This->itemName)
    {
        heap_free( itemNameA );
        return E_OUTOFMEMORY;
...
static HRESULT WINAPI ItemMonikerImpl_Save(IMoniker* iface, IStream* pStm, BOOL fClearDirty)
{
    ...
    itemNameA=heap_alloc(nameLength);
    itemDelimiterA=heap_alloc(delimiterLength);
...
static HRESULT ItemMonikerImpl_Construct(ItemMonikerImpl* This, LPCOLESTR lpszDelim,LPCOLESTR lpszItem)
{
    ...
    This->itemName=heap_alloc(sizeof(WCHAR)*(sizeStr1+1));
    if (!This->itemName)
	return E_OUTOFMEMORY;
    ...
    This->itemDelimiter=heap_alloc(sizeof(WCHAR)*(sizeStr2+1));
    if (!This->itemDelimiter) {
...
HRESULT WINAPI CreateItemMoniker(LPCOLESTR lpszDelim, LPCOLESTR  lpszItem, IMoniker **ppmk)
{
    ...
    newItemMoniker = heap_alloc(sizeof(ItemMonikerImpl));
