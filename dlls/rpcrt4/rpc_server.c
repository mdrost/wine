    ...
    unsigned char **auth_data_out, ULONG *auth_length_out)
{
  ...
  results = heap_alloc(hdr->num_elements * sizeof(*results));
  if (!results)
...
static DWORD CALLBACK RPCRT4_io_thread(LPVOID the_arg)
{
  ...
  for (;;) {
    msg = heap_alloc_zero(sizeof(RPC_MESSAGE));
    if (!msg) break;
    ...
    case PKT_REQUEST:
      ...
      packet = heap_alloc(sizeof(RpcPacket));
      if (!packet) {
        I_RpcFree(msg->Buffer);
        RPCRT4_FreeHeader(hdr);
        heap_free(msg);
...
RPC_STATUS WINAPI RpcServerInqBindings( RPC_BINDING_VECTOR** BindingVector )
{
  ...
  if (count) {
    /* export bindings */
    *BindingVector = heap_alloc(sizeof(RPC_BINDING_VECTOR) +
    ...
    UINT Flags, UINT MaxCalls, UINT MaxRpcSize, RPC_IF_CALLBACK_FN* IfCallbackFn, void* SecurityDescriptor)
{
  ...
  sif = heap_alloc_zero(sizeof(RpcServerInterface));
  sif->If           = If;
  if (MgrTypeUuid) {
...
RPC_STATUS WINAPI RpcObjectSetType( UUID* ObjUuid, UUID* TypeUuid )
{
  ...
  if ((! TypeUuid) || UuidIsNil(TypeUuid, &dummy)) {
    ...
    map = heap_alloc(sizeof(RpcObjTypeMap));
    map->Object = *ObjUuid;
    map->Type = *TypeUuid;
    map->next = NULL;
    if (prev)
...
void RPCRT4_ServerFreeAllRegisteredAuthInfo(void)
{
    ...
    {
        heap_free(auth_info->package_name);
        heap_free(auth_info->principal);
        heap_free(auth_info);
    }
    LeaveCriticalSection(&server_auth_info_cs);
                            ...
                            LPVOID Arg )
{
    ...
    auth_info = heap_alloc_zero(sizeof(*auth_info));
    if (!auth_info) {
...
RPC_STATUS RPC_ENTRY RpcServerInqDefaultPrincNameW(ULONG AuthnSvc, RPC_WSTR *PrincName)
{
    ...
    if (!(*PrincName = heap_alloc( len * sizeof(WCHAR) )))
...
RPC_STATUS WINAPI RpcMgmtInqStats(RPC_BINDING_HANDLE Binding, RPC_STATS_VECTOR **Statistics)
{
  ...
  if ((stats = heap_alloc(sizeof(RPC_STATS_VECTOR))))
  {
    stats->Count = 1;
    stats->Stats[0] = 0;
    *Statistics = stats;
