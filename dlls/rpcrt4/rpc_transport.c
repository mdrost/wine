...
{
  RpcConnection_np *npc = heap_alloc_zero(sizeof(RpcConnection_np));
  return &npc->common;
}
...
static RPC_STATUS rpcrt4_ncacn_np_handoff(RpcConnection *old_conn, RpcConnection *new_conn)
{
  ...
  new_conn->NetworkAddr = heap_alloc(len);
...
{
    RpcServerProtseq_np *ps = heap_alloc_zero(sizeof(*ps));
    if (ps)
...
static void *rpcrt4_protseq_np_get_wait_array(RpcServerProtseq *protseq, void *prev_array, unsigned int *count)
{
    ...
    *count = 1;
    ...
    if (objs)
        objs = heap_realloc(objs, *count*sizeof(HANDLE));
    else
        objs = heap_alloc(*count*sizeof(HANDLE));
    if (!objs)
    {
...
static RpcConnection *rpcrt4_conn_tcp_alloc(void)
{
  ...
  tcpc = heap_alloc_zero(sizeof(RpcConnection_tcp));
  if (tcpc == NULL)
    return NULL;
  tcpc->sock = -1;
...
static RPC_STATUS rpcrt4_conn_tcp_handoff(RpcConnection *old_conn, RpcConnection *new_conn)
{
  ...
  client->common.NetworkAddr = heap_alloc(INET6_ADDRSTRLEN);
...
{
    RpcServerProtseq_sock *ps = heap_alloc_zero(sizeof(*ps));
    if (ps)
    {
        static BOOL wsa_inited;
        if (!wsa_inited)
...
static ULONG RpcHttpAsyncData_Release(RpcHttpAsyncData *data)
{
    ...
    if (!refs)
    {
        ...
        heap_free(data->inet_buffers.lpvBuffer);
...
static RpcConnection *rpcrt4_ncacn_http_alloc(void)
{
    ...
    httpc = heap_alloc_zero(sizeof(*httpc));
    if (!httpc) return NULL;
    httpc->async_data = heap_alloc_zero(sizeof(RpcHttpAsyncData));
    if (!httpc->async_data)
    {
        heap_free(httpc);
        return NULL;
...
static RPC_STATUS rpcrt4_http_check_response(HINTERNET hor)
{
    ...
    {
        status_text = heap_alloc(size);
        ret = HttpQueryInfoW(hor, HTTP_QUERY_STATUS_TEXT, status_text, &size, &index);
    }

    ERR("server returned: %d %s\n", status_code, ret ? debugstr_w(status_text) : "<status text unavailable>");
    if(status_text != buf) heap_free(status_text);
...
static RPC_STATUS rpcrt4_http_internet_connect(RpcConnection_http *httpc)
{
    ...
    if (!servername)
    {
        servername = heap_alloc((strlen(httpc->common.NetworkAddr) + 1)*sizeof(WCHAR));
        if (!servername)
        {
            heap_free(password);
            heap_free(user);
                                  ...
                                  void *buffer, unsigned int count)
{
    ...
    async_data->inet_buffers.lpvBuffer = heap_alloc(count);
    ...
    heap_free(async_data->inet_buffers.lpvBuffer);
                                   ...
                                   const RPC_HTTP_TRANSPORT_CREDENTIALS_W *creds, struct authinfo **auth_ptr)
{
    ...
    case RPC_C_HTTP_AUTHN_SCHEME_BASIC:
    {
        ...
        if (!(info->data = heap_alloc(info->data_len)))
        {
    ...
    case RPC_C_HTTP_AUTHN_SCHEME_NEGOTIATE:
    {
        ...
        if (!first && *p == ' ')
        {
            ...
            if (!(in.pvBuffer = heap_alloc(in.cbBuffer))) break;
            decode_base64(p, len, in.pvBuffer);
        }
        out.BufferType = SECBUFFER_TOKEN;
        out.cbBuffer   = info->max_token;
        if (!(out.pvBuffer = heap_alloc(out.cbBuffer)))
        {
...
static RPC_STATUS insert_authorization_header(HINTERNET request, ULONG scheme, char *data, int data_len)
{
    ...
    if ((header = heap_alloc((auth_len + scheme_len + len + 2) * sizeof(WCHAR))))
    {
...
static RPC_STATUS set_auth_cookie(RpcConnection_http *httpc, const WCHAR *value)
{
    ...
    if (!(url = heap_alloc(len))) return RPC_S_OUT_OF_MEMORY;
...
static RPC_STATUS rpcrt4_ncacn_http_open(RpcConnection* Connection)
{
    ...
    url = heap_alloc(sizeof(wszRpcProxyPrefix) + (strlen(Connection->NetworkAddr) + 1 + strlen(Connection->Endpoint))*sizeof(WCHAR));
    if (!url)
    ...
    timer_data = heap_alloc(sizeof(*timer_data));
    if (!timer_data)
...
again:
  ...
  *Header = heap_alloc(hdr_length);
  ...
  {
    *Payload = heap_alloc(common_hdr.frag_len - hdr_length);
    if (!*Payload)
    {
      status = RPC_S_OUT_OF_RESOURCES;
      goto fail;
...
void RPCRT4_ReleaseConnection(RpcConnection *connection)
{
    ...
    if (!ref)
    {
        ...
        heap_free(connection->NetworkOptions);
        heap_free(connection->CookieAuth);
...
RPC_STATUS WINAPI RpcNetworkInqProtseqsW( RPC_PROTSEQ_VECTORW** protseqs )
{
  ...
  *protseqs = heap_alloc(sizeof(RPC_PROTSEQ_VECTORW)+(sizeof(unsigned short*)*ARRAYSIZE(protseq_list)));
  if (!*protseqs)
    goto end;
  pvector = *protseqs;
  pvector->Count = 0;
  for (i = 0; i < ARRAYSIZE(protseq_list); i++)
  {
    pvector->Protseq[i] = heap_alloc((strlen(protseq_list[i].name)+1)*sizeof(unsigned short));
...
RPC_STATUS WINAPI RpcNetworkInqProtseqsA(RPC_PROTSEQ_VECTORA** protseqs)
{
  ...
  *protseqs = heap_alloc(sizeof(RPC_PROTSEQ_VECTORW)+(sizeof(unsigned char*)*ARRAYSIZE(protseq_list)));
  if (!*protseqs)
    goto end;
  pvector = *protseqs;
  pvector->Count = 0;
  for (i = 0; i < ARRAYSIZE(protseq_list); i++)
  {
    pvector->Protseq[i] = heap_alloc(strlen(protseq_list[i].name)+1);
    if (pvector->Protseq[i] == NULL)
      goto end;
