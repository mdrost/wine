...
null_list(void)
{
  test_list_t *n = heap_alloc(sizeof *n);
  n->t = TL_NULL;
  n->u.x = 0;
  return n;
}
...
{
  wire_bstr_t wb = (wire_bstr_t) buffer;
  short *data = heap_alloc((wb->n + 1) * sizeof *data);
  data[0] = wb->n;
...
pointer_tests(void)
{
  ...
  pus.n = 4;
  pus.ps = heap_alloc(pus.n * sizeof pus.ps[0]);
  pus.ps[0] = xstrdup("5");
  pus.ps[1] = xstrdup("6");
  pus.ps[2] = xstrdup("7");
  pus.ps[3] = xstrdup("8");
  ...
  heap_free(pus.ps);
  cpus.n = 4;
  cpus.ps = heap_alloc(cpus.n * sizeof cpus.ps[0]);
  cpus.ps[0] = xstrdup("5");
  cpus.ps[1] = xstrdup("6");
  cpus.ps[2] = xstrdup("7");
  cpus.ps[3] = xstrdup("8");
  ...
  if (!old_windows_version)
  {
      ...
      name.size = 10;
      name.name = buffer = heap_alloc_zero(name.size);
      get_name(&name);
...
array_tests(void)
{
  ...
  cs = heap_alloc(FIELD_OFFSET(cs_t, ca[5]));
  cs->n = 5;
  cs->ca[0] = 3;
  cs->ca[1] = 5;
  cs->ca[2] = -2;
  ...
  dc = heap_alloc(FIELD_OFFSET(doub_carr_t, a[2]));
  dc->n = 2;
  dc->a[0] = heap_alloc(FIELD_OFFSET(doub_carr_1_t, a[3]));
  dc->a[0]->n = 3;
  dc->a[0]->a[0] = 5;
  dc->a[0]->a[1] = 1;
  dc->a[0]->a[2] = 8;
  dc->a[1] = heap_alloc(FIELD_OFFSET(doub_carr_1_t, a[2]));
  dc->a[1]->n = 2;
  dc->a[1]->a[0] = 2;
  dc->a[1]->a[1] = 3;
  ...
  *pi = -1;
  ...
  if (!old_windows_version)
  {
      ns = heap_alloc_zero(FIELD_OFFSET(numbers_struct_t, numbers[5]));
      ns->length = 5;
      ns->size = 5;
      ns->numbers[0].pi = pi;
...
START_TEST(server)
{
  ...
  domain_and_user = heap_alloc(size);
