...
static int mp_grow (mp_int * a, int size)
{
  ...
  if (a->alloc < size) {
     ...
     */
    tmp = heap_realloc(a->dp, sizeof (mp_digit) * size);
    if (tmp == NULL) {
...
static int mp_init (mp_int * a)
{
  ...
  a->dp = heap_alloc(sizeof (mp_digit) * MP_PREC);
  if (a->dp == NULL) {
    return MP_MEM;
  }
...
static int mp_init_size (mp_int * a, int size)
{
  ...
  /* alloc mem */
  a->dp = heap_alloc(sizeof (mp_digit) * size);
  if (a->dp == NULL) {
    return MP_MEM;
  }
...
int mp_shrink (mp_int * a)
{
  ...
  if (a->alloc != a->used && a->used > 0) {
    if ((tmp = heap_realloc(a->dp, sizeof (mp_digit) * a->used)) == NULL) {
      return MP_MEM;
    }
    a->dp    = tmp;
    a->alloc = a->used;
