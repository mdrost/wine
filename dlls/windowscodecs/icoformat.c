...
static HRESULT ReadIcoDib(IStream *stream, IcoFrameDecode *result)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                result->bits = heap_alloc(result->width * result->height * 4);
...
static HRESULT ReadIcoPng(IStream *stream, IcoFrameDecode *result)
{
    ...
    if (FAILED(hr))
        goto end;
    result->bits = heap_alloc(4 * result->width * result->height);
    if (result->bits == NULL)
    {
        hr = E_OUTOFMEMORY;
        goto end;
    ...
    UINT index, IWICBitmapFrameDecode **ppIBitmapFrame)
{
    ...
    result = heap_alloc(sizeof(IcoFrameDecode));
    if (!result)
    {
        hr = E_OUTOFMEMORY;
        goto fail;
...
HRESULT IcoDecoder_CreateInstance(REFIID iid, void** ppv)
{
    ...
    *ppv = NULL;
    ...
    This = heap_alloc(sizeof(IcoDecoder));
