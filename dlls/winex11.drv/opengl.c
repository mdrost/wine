...
static BOOL X11DRV_WineGL_InitOpenglInfo(void)
{
    ...
    WineGLInfo.glExtensions = heap_alloc(strlen(str)+sizeof(legacy_extensions));
...
static void init_pixel_formats( Display *display )
{
    ...
    list = heap_alloc_zero((nCfgs + bmp_formats) * sizeof(*list));
...
static HDC X11DRV_wglGetPbufferDCARB( struct wgl_pbuffer *object )
{
    ...
    if (!(gl = heap_alloc_zero( sizeof(*gl) )))
    {
        DeleteDC( hdc );
        return 0;
    }
