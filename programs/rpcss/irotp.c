    ...
    IrotContextHandle *ctxt_handle)
{
    ...
    rot_entry->refs = 1;
    rot_entry->object = heap_alloc(FIELD_OFFSET(InterfaceData, abData[obj->ulCntData]));
    if (!rot_entry->object)
    {
    ...
    rot_entry->moniker = heap_alloc(FIELD_OFFSET(InterfaceData, abData[mk->ulCntData]));
    ...
    rot_entry->moniker_data = heap_alloc(FIELD_OFFSET(MonikerComparisonData, abData[data->ulCntData]));
