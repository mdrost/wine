...
static char* heap_printf(const char *format, ...)
{
    ...
    ret = heap_realloc(buffer, strlen(buffer) + 1 );
    if (!ret) ret = buffer;
    return ret;
}
...
static IStream *add_module_icons_to_stream(struct IconData16 *iconData16, HMODULE hModule, GRPICONDIR *grpIconDir)
{
    ...
    iconDirEntries = heap_alloc(grpIconDir->idCount*sizeof(ICONDIRENTRY));
...
static HRESULT read_ico_direntries(IStream *icoStream, ICONDIRENTRY **ppIconDirEntries, int *numEntries)
{
    ...
    *numEntries = iconDir.idCount;
    ...
    if ((*ppIconDirEntries = heap_alloc(sizeof(ICONDIRENTRY)*iconDir.idCount)) == NULL)
    {
        hr = E_OUTOFMEMORY;
        goto end;
    }
...
static WCHAR* assoc_query(ASSOCSTR assocStr, LPCWSTR name, LPCWSTR extra)
{
    ...
    if (SUCCEEDED(hr))
    {
        value = heap_alloc(size * sizeof(WCHAR));
        if (value)
        {
...
static BOOL write_menu_file(const char *unix_link, const char *filename)
{
    ...
    name = heap_alloc(lstrlenA(filename) + 1);
...
static LPSTR escape(LPCWSTR arg)
{
    ...
    escaped_string = heap_alloc((4 * strlenW(arg) + 1) * sizeof(WCHAR));
                            ...
                            LPWSTR szArgs, DWORD argsSize)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        {
            cmdSize++;
            szCmdline = heap_alloc( cmdSize*sizeof(WCHAR) );
...
{
    int i;
    char *ret = heap_alloc(lstrlenA(string) + 1);
    if (ret)
    {
        for (i = 0; string[i]; i++)
        {
    ...
    if (globs_filename)
    {
        ...
        if (globs_file) /* doesn't have to exist */
        {
            ...
            while (ret && (ret = next_line(globs_file, &line, &size)) && line)
            {
                ...
                {
                    mime_type_entry = heap_alloc(sizeof(struct xdg_mime_type));
                    if (mime_type_entry)
                    {
                        *pos = 0;
                                                ...
                                                char **mime_type)
{
    ...
    lower_extensionW = heap_alloc((len + 1)*sizeof(WCHAR));
    if (lower_extensionW)
    {
    ...
    if ((assocKey = open_associations_reg_key()))
    {
        ...
        for (i = 0; !done;)
        {
            ...
            do
            {
                ...
                extensionW = heap_alloc(size * sizeof(WCHAR));
                if (extensionW == NULL)
                {
                    WINE_ERR("out of memory\n");
    ...
    for (i = 0; ; i++)
    {
        ...
        if (ret == ERROR_SUCCESS && extensionW[0] == '.' && !is_extension_blacklisted(extensionW))
        {
            ...
            if (mimeProgId)
            {
                ...
                entry = heap_alloc(sizeof(struct rb_string_entry));
                if (!entry)
                {
    ...
    if (hkey)
    {
        ...
        for (i = 0; lret == ERROR_SUCCESS; )
        {
            ...
            while (1)
            {
                ...
                value = heap_alloc(valueSize * sizeof(WCHAR));
                if (value == NULL)
                    break;
                data = heap_alloc(dataSize * sizeof(WCHAR));
                if (data == NULL)
                    break;
